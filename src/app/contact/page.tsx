    ...
    <div className="space-y-20 pb-24 pt-24">
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-4xl space-y-6 px-4 text-center sm:px-6">
          ...
          />
          <FlagStrip flags={accentFlags} className="justify-center" size={34} />
        </div>
      </section>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-6 rounded-[2.5rem] border border-white/12 bg-[rgba(26,0,3,0.85)] p-8 shadow-[0_24px_60px_rgba(59,0,4,0.5)]">
            ...
            <div className="space-y-4 text-sm text-muted">
              <div>
                ...
                <Link href={`mailto:${contact.email}`} className="text-gold">
                  {contact.email}
                </Link>
              </div>
              <div>
            ...
            <div className="space-y-4">
              <h4 className="text-lg font-semibold text-white">Let us know what you’re planning</h4>
              <ul className="space-y-2 text-sm text-muted">
                {inquiryTypes.map((item) => (
                  <li key={item.title} className="rounded-2xl border border-white/10 bg-[rgba(53,1,4,0.65)] px-4 py-3">
          ...
          <div className="space-y-6">
            <div className="rounded-[2.5rem] border border-white/12 bg-[rgba(26,0,3,0.85)] p-8 shadow-[0_24px_60px_rgba(59,0,4,0.5)]">
              ...
              <div className="mt-5 flex flex-wrap gap-3 text-sm text-gold">
                {socialLinks.slice(0, 3).map((social) => (
                  <Link
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2 rounded-full border border-white/15 px-4 py-2 transition hover:border-white/40 hover:bg-[rgba(243,193,68,0.12)]"
                  >
                    {social.label}
                  </Link>
                ))}
